...
| SYNC-D | Real PostgreSQL + multi-entity — generic `cloud_events` feed for inventory/audit/payment, real migration runner, concurrency/rollback proven against REAL PostgreSQL, async HTTP server over SQLite-dev/Postgres-prod, deficit status workflow, Windows key-storage validation script. Cloud not yet deployed to a hosted provider. [`SYNC_D_MULTI_ENTITY.md`](./SYNC_D_MULTI_ENTITY.md) | ✅ Done |
