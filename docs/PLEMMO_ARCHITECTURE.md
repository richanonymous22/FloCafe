...
| `cloud/*` (`migrate.ts`, `run-migrations.ts`, generalized `store.ts`/`server.ts`) + `sync/audit-events.ts`, `sync/payment-events.ts` | ✅ Built (SYNC-D) | Real-Postgres + multi-entity — generic `cloud_events` feed for inventory/audit/payment, async HTTP server over sync-SQLite or async-Postgres, real migration runner, deficit status workflow. Proven against REAL PostgreSQL (concurrency, rollback) + real HTTP |
| `sync/sales-events.ts` + generalized outbox/store + `remote_*` mirrors (v88) + `cloud/migrations/postgres/0002_sales.sql` | ✅ Built (SYNC-E) | Sales sync — versioned snapshots of order/order_item/bill through the shared seam, mutable-snapshot outbox, cloud version projection + conflict detection (`cloud_conflicts`), remote apply into FK-free mirrors (never authoritative), missing-parent staging. Proven against REAL PostgreSQL + real HTTP |
| `sync/conflict-model.ts`, `conflict-store.ts`, `conflict-resolution.ts`, `sales-reconciliation.ts`, `pending-relationships.ts`, `conflict-sync.ts` + `routes/sales-reconciliation.ts` + `sales_conflicts`/`sales_reconciliation_actions`/`sales_pending_relationships` (v89) + `cloud/migrations/postgres/0003_conflicts.sql` | ✅ Built (SYNC-F) | Sales conflict resolution + reconciliation — closed conflict taxonomy + state machine, authorized/financial-safe resolution engine (`sales.reconcile`, owner/manager), compensation-as-record, sales/payment/inventory reconciliation, durable missing-parent recovery, cloud conflict pull/resolve with server-side re-validation. Never mutates an authoritative sale. Proven against REAL PostgreSQL + real HTTP |
| `core/admin/{sales-overview,conflict-console,inventory-reconciliation,sync-health}.ts` + `routes/admin-reconciliation.ts` + cloud `GET /sync/v1/{health,deficits}` + `frontend .../reconciliation/page.tsx` | ✅ Built (SYNC-G) | Admin sales console + cross-device balance reconciliation — operator read models (paginated sales overview, conflict queue/detail with local/remote sides + financial significance, inventory discrepancy with KNOWN/LIKELY/UNKNOWN evidence, sync/device health), safe resolution actions, compensation `manual_action_required` state (no auto refund/void/stock mutation), and a minimal owner/manager Reconciliation UI. No new migration. Proven against REAL PostgreSQL + real HTTP |
| `core/sync/reference-entities.ts` + `remote_reference_entities` (v90) + `core/licensing.ts` + `core/admin/compensation.ts` + `cloud/{serve.ts,Dockerfile}` + cloud `/health` `/ready` `X-Plemmo-Protocol` | ✅ Built (COMMERCIALIZATION) | Catalog/customer/supplier/PO/transfer sync as versioned reference snapshots into a generic mirror (per-entity conflict policy; mirror-only, ULID-idempotent, never overwrites the authoritative catalog); safe exactly-once refund/void compensation execution (inventory correction honestly manual); licensing foundation (states + offline grace + feature entitlement over the existing feature system); production deploy artifacts (server entrypoint, container, health/readiness, protocol version). Permanent cloud deployment still needs external provisioning. Proven against REAL PostgreSQL + real HTTP |
...
| Synchronization | ⚠️ Sales conflict resolution + reconciliation, real-PG (SYNC-F) | `sync_outbox`/`sync_inbox`/`sync_state` (v86), `remote_payment_events` (v87), `remote_orders`/`remote_order_items`/`remote_bills` (v88), `sales_conflicts`/`sales_reconciliation_actions`/`sales_pending_relationships` (v89); cloud `cloud_events`/`cloud_entity_versions`/`cloud_conflicts`/`cloud_conflict_resolutions` | inventory_movement + audit_event + payment_event + order + order_item + bill sync through one protocol/feed. Mutable sales use versioned snapshots + cross-device conflict detection; remote apply into FK-free mirrors (a local completed sale is never mutated by sync). SYNC-F adds an authorized, financial-safe conflict-resolution engine + sales/payment/inventory reconciliation + durable missing-parent recovery. Proven against REAL PostgreSQL + real HTTP. Products/customers/suppliers/etc. NOT synced; no Admin UI. Hosted **Neon PostgreSQL** connectivity + the SYNC-D hosted-DB test are proven (later verification, 2026-08-17 — 43/43 from the real Windows client), but a permanently operated cloud deployment (public API/domain/CI-CD/monitoring/backup-restore) is not yet stood up. See `SYNC_F_CONFLICT_RECONCILIATION.md`, `SYNC_E_SALES_ARCHITECTURE.md`, `cloud/DEPLOYMENT.md` and §7 |
...
| SYNC-D | Real PostgreSQL + multi-entity — generic `cloud_events` feed for inventory/audit/payment, real migration runner, concurrency/rollback proven against REAL PostgreSQL, async HTTP server over SQLite-dev/Postgres-prod, deficit status workflow, Windows key-storage validation script. Hosted **Neon PostgreSQL** connectivity + the hosted-DB test proven (later verification, 2026-08-17 — 43/43 from the real Windows client); a permanently operated cloud deployment is not yet stood up. [`SYNC_D_MULTI_ENTITY.md`](./SYNC_D_MULTI_ENTITY.md) | ✅ Done |
| SYNC-E | Sales / orders synchronization — versioned snapshots of order/order_item/bill through the shared multi-entity seam, mutable-snapshot outbox, cloud version projection + cross-device conflict detection, remote apply into FK-free mirrors (completed sales never undone), missing-parent staging. Proven against REAL PostgreSQL + real HTTP. [`SYNC_E_SALES_ARCHITECTURE.md`](./SYNC_E_SALES_ARCHITECTURE.md) | ✅ Done |
| SYNC-F | Sales conflict resolution + reconciliation — closed conflict taxonomy + state machine, authorized/financial-safe resolution engine (`sales.reconcile`, owner/manager), compensation-as-durable-record, sales/payment/inventory reconciliation, durable missing-parent recovery, cloud conflict pull/resolve with server-side re-validation; never mutates an authoritative sale. Proven against REAL PostgreSQL + real HTTP. [`SYNC_F_CONFLICT_RECONCILIATION.md`](./SYNC_F_CONFLICT_RECONCILIATION.md) | ✅ Done |
| SYNC-G | Admin sales console + cross-device balance reconciliation — operator read models + `/api/admin` (paginated sales overview, conflict queue/detail, inventory discrepancy with KNOWN/LIKELY/UNKNOWN evidence, sync/device health), safe resolution actions, compensation `manual_action_required` (no auto refund/void/stock mutation), org-scoped cloud health/deficit endpoints, and a minimal owner/manager Reconciliation UI. Proven against REAL PostgreSQL + real HTTP. [`SYNC_G_ADMIN_RECONCILIATION.md`](./SYNC_G_ADMIN_RECONCILIATION.md) | ✅ Done |
| COMMERCIALIZATION | Catalog/customer/supplier/PO/transfer sync (versioned reference snapshots → generic mirror, per-entity conflict policy, ULID-idempotent, mirror-only); safe exactly-once refund/void compensation; licensing foundation (offline grace + feature entitlement); production deploy artifacts (server entrypoint, container, health/readiness, protocol version). Permanent cloud deployment still needs external provisioning. Proven against REAL PostgreSQL + real HTTP. [`COMMERCIAL_PLATFORM_COMPLETION.md`](./COMMERCIAL_PLATFORM_COMPLETION.md) | ✅ Done |
| PLATFORM-HARDENING | Final backend milestone: remaining catalog emitters (category/variant/addon_group/delete paths); controlled catalog promotion (INSERT-if-absent, protects local edits, never overwrites); AUTOMATED exactly-once inventory compensation (`adjustStock` idempotency key); cloud licensing verification (`cloud_licenses` + `GET /sync/v1/license` + offline-grace verifier); consolidated security + migration audit. Hard boundary: no further core backend feature expansion except real bug/security fixes. Proven against REAL PostgreSQL + real HTTP. [`PLATFORM_HARDENING_FINAL.md`](./PLATFORM_HARDENING_FINAL.md) | ✅ Done |
| SYNC-F+ | Conflict resolution + Admin sales console APIs, execute the managed-Postgres deployment, run Windows DPAPI validation on real hardware, then extend sync to products/customers/suppliers — not started | Next |
...
- [`SYNC_D_MULTI_ENTITY.md`](./SYNC_D_MULTI_ENTITY.md) — real PostgreSQL + multi-entity: the SYNC-C audit, the generic `cloud_events` feed for inventory/audit/payment, the real migration runner, concurrency + rollback proven against REAL PostgreSQL, the async HTTP server over SQLite-dev/Postgres-prod, the deficit status workflow, and the honest real-vs-simulated split (deployment + Windows DPAPI). See also [`cloud/DEPLOYMENT.md`](../cloud/DEPLOYMENT.md)
- [`SYNC_E_SALES_ARCHITECTURE.md`](./SYNC_E_SALES_ARCHITECTURE.md) — sales synchronization: the adversarial write-path audit, the versioned-snapshot model for mutable order/order_item/bill, identity + relationship + lifecycle, the conflict-detection model (record-not-merge, never undo a completed sale), remote apply into FK-free mirrors, missing-parent staging, transactional atomicity, and the hospitality/retail/inventory guarantees preserved
- [`SYNC_F_CONFLICT_RECONCILIATION.md`](./SYNC_F_CONFLICT_RECONCILIATION.md) — sales conflict resolution + reconciliation: the SYNC-E audit, the closed conflict taxonomy + state machine, the financial-safety rule, the authorized resolution engine (`sales.reconcile`), compensation-as-record, the sales/payment/inventory reconciliation service, durable missing-parent recovery, the cloud/local ownership split, and the security/atomicity guarantees
- [`SYNC_G_ADMIN_RECONCILIATION.md`](./SYNC_G_ADMIN_RECONCILIATION.md) — admin sales console + cross-device balance reconciliation: the SYNC-F audit + operator-readiness classification, the admin read models + `/api/admin` contract, the conflict queue/detail, safe resolution actions + the compensation `manual_action_required` STOP-SHORT, the inventory discrepancy model with KNOWN/LIKELY/UNKNOWN evidence, sync/device health, the authorization/security posture, and the minimal Reconciliation UI
- [`COMMERCIAL_PLATFORM_COMPLETION.md`](./COMMERCIAL_PLATFORM_COMPLETION.md) — the bundled commercialization milestone: production cloud status (proven vs external-blocked), the safe-compensation audit, catalog/customer/supplier/operations sync + per-entity conflict policies, the licensing foundation, merchant-onboarding readiness, the launch checklist, and known limitations
- [`PLATFORM_HARDENING_FINAL.md`](./PLATFORM_HARDENING_FINAL.md) — the final backend/platform milestone: the FINAL PLATFORM AUDIT (every limitation classified DONE / DEFER / EXTERNAL / MUST-FIX), remaining catalog emitters, controlled catalog promotion, automated exactly-once inventory compensation, cloud licensing verification, the consolidated security + migration audit, and the hard boundary + gap analysis before the UI/UX redesign
