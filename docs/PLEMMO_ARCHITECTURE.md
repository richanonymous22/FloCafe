...
| Synchronization | ⚠️ Sales + multi-entity, real-PG (SYNC-E) | `sync_outbox`/`sync_inbox`/`sync_state` (v86), `remote_payment_events` (v87), `remote_orders`/`remote_order_items`/`remote_bills` (v88); cloud `cloud_events`/`cloud_entity_versions`/`cloud_conflicts` | inventory_movement + audit_event + payment_event + order + order_item + bill sync through one protocol/feed. Mutable sales use versioned snapshots + cross-device conflict detection; remote apply into FK-free mirrors (a local completed sale is never mutated by sync). Proven against REAL PostgreSQL + real HTTP. Products/customers/suppliers/etc. NOT synced; cloud not yet deployed to a hosted provider. See `SYNC_E_SALES_ARCHITECTURE.md`, `SYNC_D_MULTI_ENTITY.md`, `cloud/DEPLOYMENT.md` and §7 |
