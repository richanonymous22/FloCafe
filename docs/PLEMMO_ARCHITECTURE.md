...
| Synchronization | ⚠️ Multi-entity, real-PG (SYNC-D) | `sync_outbox`/`sync_inbox`/`sync_state` (v86), `remote_payment_events` (v87); cloud-side Postgres `cloud_events` | inventory_movement + audit_event + payment_event sync through one protocol/feed. Proven against REAL PostgreSQL (concurrency, rollback) + real HTTP; async server over SQLite-dev/Postgres-prod. Sales/products/customers/etc. NOT synced. Cloud not yet deployed to a hosted provider. See `SYNC_D_MULTI_ENTITY.md`, `cloud/DEPLOYMENT.md` and §7 |
...
| SYNC-E+ | Execute the managed-Postgres deployment, run Windows DPAPI validation on real hardware, Admin token issuance + device/deficit console APIs, then sales/orders sync with a conflict model — not started | Next |
