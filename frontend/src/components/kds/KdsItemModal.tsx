    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-muted flex items-center justify-center text-muted-foreground hover:bg-muted shrink-0"
            ...
            {STATUS_ORDER.map((s, i) => {
              ...
              return (
                <div key={s} className="flex items-center gap-1.5">
                  ...
                  {i < STATUS_ORDER.length - 1 && <ChevronRight size={12} className="text-muted-foreground/60 shrink-0" />}
                </div>
              );
            })}
          </div>
            ...
            <>
              ...
              {prev && (
                <button
                  ...
                  disabled={updating}
                  className="w-full py-4 rounded-2xl text-muted-foreground text-base font-semibold border-2 border-border bg-muted/50 hover:bg-muted transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  <ChevronLeft size={18} />
