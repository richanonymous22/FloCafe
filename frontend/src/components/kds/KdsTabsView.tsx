        ...
        {(Object.keys(STATUS_CONFIG) as KitchenStatus[]).map((status) => {
          ...
          return (
            ...
            >
              ...
              {statusLabel(status)}
              <span className="text-xs px-1.5 py-0.5 rounded-full bg-surface/60">{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 items-start">
          {filteredOrders.map((order) => (
            <div
              key={order.id}
              className={`bg-surface rounded-xl border-2 ${STATUS_CONFIG[activeTab].border} p-4 flex flex-col`}
            >
                        ...
                        <div className="ml-[26px] flex flex-wrap gap-1 mt-1">
                          {item.addons.map((addon, i) => (
                            <span
                              ...
                              className="text-[10px] bg-surface/70 text-blue-600 px-1.5 py-0.5 rounded border border-blue-200"
                            >
